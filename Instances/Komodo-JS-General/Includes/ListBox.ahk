...
InitializeListBox()
{
   ...
   Gui, ListBoxGui: -DPIScale -Caption +AlwaysOnTop +ToolWindow +Delimiter%g_DelimiterChar%
	;Gui, ListBoxGui:color,, caeec5 ; VILBUR: edited background color
	Gui, ListBoxGui:color,, ced8787 ; VILBUR: edited background color
...
AddToMatchList(position, MaxLength, HalfLength, LongestBaseLength, ComputeBaseLengthOnly)
{
   ...
   if (AdditionalDataExists)
   {
      ...
      {
		;;; VILBUR: line 432
		if(g_SingleMatchReplacement[position] != RegExReplace( CurrentMatch, "\.", "_" )) ;;; don`t show replacement, if only differnece between match and replacement are '.' & '_' E.G: CurrentMatch='Foo.Bar' & Replacement='Foo_Bar'
      ...
      if (g_SingleMatchDescription[position])
      {
         ...
         CurrentMatch .= Tabs . "`t|" . g_SingleMatchDescription[position]
      }
